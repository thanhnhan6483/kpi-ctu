...
import { FileText, CheckCircle, Clock, AlertTriangle, Search, Plus, Trash2, Eye, Link } from 'lucide-react';
...
interface Evidence {
  ...
  submittedAt: string; submittedBy: string; version: number; classification: 'normal' | 'internal' | 'confidential';
}
...
export default function EvidencesPage() {
  ...
  const handleReview = async (id: string, status: string, note: string) => { await apiPut(`/api/evidences/${id}`, { status, reviewerNote: note, reviewedBy: 'u001', reviewedAt: new Date().toISOString() }); setShowReview(false); load(); };
  ...
  const Form = ({ onSubmit }: { onSubmit: (d: Partial<Evidence>) => void }) => {
    const [form, setForm] = useState({ planItemIds: [] as string[], evidenceType: 'file' as const, fileName: '', externalUrl: '', submittedBy: 'u002', classification: 'normal' as const });
    return (
      <form onSubmit={e => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
        ...
        <div><label className="block text-sm font-medium mb-1">Phân loại bảo mật</label><select value={form.classification} onChange={e => setForm({ ...form, classification: e.target.value as any })} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:border-primary"><option value="normal">Bình thường</option><option value="internal">Nội bộ</option><option value="confidential">Nhạy cảm</option></select></div>
      ...
      <div className="card">
        ...
        <div className="p-0">
          <div className="overflow-x-auto"><table className="table">
            <thead><tr><th>STT</th><th>Loại</th><th>Tệp/URL</th><th>KPI liên kết</th><th>Phân loại</th><th>Trạng thái</th><th>Ngày nộp</th><th>Thao tác</th></tr></thead>
            <tbody>
              {loading ? <tr><td colSpan={8} className="text-center py-8">Đang tải...</td></tr> :
              filtered.length === 0 ? <tr><td colSpan={8} className="text-center py-8">Chưa có minh chứng</td></tr> :
              filtered.map((item, idx) => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td><span className={`badge ${item.classification === 'confidential' ? 'badge-danger' : item.classification === 'internal' ? 'badge-warning' : 'badge-info'}`}>{item.classification === 'confidential' ? 'Nhạy cảm' : item.classification === 'internal' ? 'Nội bộ' : 'Bình thường'}</span></td>
